...
GitHub Actions sets `CRAFT_REQUIRE_API=true`, so an unavailable or invalid Craft API stops the deployment instead of publishing fallback content. OVH paths are validated against one-time root markers before every sync, all root dotfiles and `cms/` are protected, and recovery data for the ten latest deployments is kept outside the webroot. Set the optional `SITE_URL` repository variable to an HTTPS public URL to verify the exact deployed commit after each run.
